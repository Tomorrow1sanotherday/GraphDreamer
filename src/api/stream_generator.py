...
class StreamGenerator:
    ...
    async def generate_stream_with_index(
        ...
        """
        Generate responses for a list of prompts with index in a streaming fashion.

        Args:
            prompts_with_index: List of tuples (index, prompt)
            system_prompt: System prompt (same for all requests)
            validate_func: Optional function to validate the response.

        Yields:
            Tuple containing index and response data
        """
        tasks = set()
    ...
    async def _generate_single_with_index(
        ...
        """
        Generate a single response with retry mechanism and keep track of index.
        ...
        Returns:
            Tuple containing index and parsed JSON response or None if failed
        """
        retry_count = 0
